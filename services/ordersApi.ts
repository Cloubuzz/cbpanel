...
export const fetchOrders = async ({
  ...
  const endpoint = `orders/orders?status=${encodeURIComponent(status)}&page=${page}&pageSize=${pageSize}`;
  const response = await adminRequest<OrdersApiResponse>(token, endpoint);
...
export const fetchOrderDetail = async (orderId: number, token: string): Promise<OrderDetail> => {
  const response = await adminRequest<OrderDetailApiResponse>(token, `orders/${orderId}`);
...
export const fetchOrderLogs = async (orderId: number, token: string): Promise<OrderLogs> => {
  const response = await adminRequest<OrderLogsApiResponse>(token, `orders/${orderId}/logs`);
  if (response.responseType !== 1 || !response.data) {
...
export const fetchPaymentLogs = async (orderId: number, token: string): Promise<PaymentLog[]> => {
  const response = await adminRequest<PaymentLogsApiResponse>(token, `orders/${orderId}/paymentlogs`);
...
export const fetchOutlets = async (city: string, token: string): Promise<Outlet[]> => {
  const endpoint = `orders/outlets?city=${encodeURIComponent(city)}`;
  const response = await adminRequest<OutletsApiResponse>(token, endpoint);
  if (response.responseType !== 1 || !Array.isArray(response.data)) {
...
export const processOrder = async (payload: ProcessOrderPayload, token: string): Promise<void> => {
  const response = await adminRequest<ProcessOrderApiResponse>(token, `orders/process`, 'POST', payload);
  if (!response.success) {
