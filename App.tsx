...
const AutomationManager = lazyNamed(() => import("./pages/AutomationManager"), "AutomationManager");
const EmailManager = lazyNamed(() => import("./pages/EmailManager"), "EmailManager");
const WhatsAppManager = lazyNamed(() => import("./pages/WhatsAppManager"), "WhatsAppManager");
...
const Modifiers = lazyNamed(() => import("./pages/Modifiers"), "Modifiers");
const Menus = lazyNamed(() => import("./pages/Menus"), "Menus");
const Categories = lazyNamed(() => import("./pages/Categories"), "Categories");
const Banners = lazyNamed(() => import("./pages/Banners"), "Banners");
const Vouchers = lazyNamed(() => import("./pages/Vouchers"), "Vouchers");
const Discounts = lazyNamed(() => import("./pages/Discounts"), "Discounts");
const Reports = lazyNamed(() => import("./pages/Reports"), "Reports");
const ReportDetail = lazyNamed(() => import("./pages/ReportDetail"), "ReportDetail");
const OutletManager = lazyNamed(() => import("./pages/OutletManager"), "OutletManager");
const OutletDetail = lazyNamed(() => import("./pages/OutletDetail"), "OutletDetail");
const HelpDesk = lazyNamed(() => import("./pages/HelpDesk"), "HelpDesk");
...
const App: React.FC = () => {
  ...
  const isLaunchpad = currentView === "launchpad";
    ...
    <div
      className={`flex h-screen w-full overflow-hidden font-sans transition-colors duration-300 ${
        isLaunchpad
          ...
          : "bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-950 dark:to-slate-900 text-slate-900 dark:text-slate-100 selection:bg-teal-500 selection:text-white"
      }`}
    >
      {/* Dark Mode Gradient Mesh (Only visible in dark mode) */}
      <div
        className={`fixed inset-0 z-0 bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-teal-900/20 via-slate-950 to-slate-950 pointer-events-none ${
          isLaunchpad ? "block" : "hidden dark:block"
        }`}
      ></div>
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative transition-all duration-300 z-10">
        {!isLaunchpad && (
          <header className="h-20 border-b border-white/5 dark:border-teal-900/10 bg-gradient-to-r from-[#020617] via-[#0f172a] to-[#020617] backdrop-blur-xl flex items-center justify-between px-6 md:px-8 z-20 transition-all duration-300 text-white">
            <div className="flex items-center gap-4">
              ...
              <button
                ...
                className="p-2.5 bg-slate-900/50 hover:bg-teal-500/10 text-teal-400 rounded-xl border border-white/5 transition-all shadow-[0_0_15px_rgba(20,184,166,0.1)] hover:shadow-[0_0_20px_rgba(20,184,166,0.2)] group"
              ...
              <div className="relative hidden md:block w-96 group">
                ...
                <Search
                  className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 group-hover:text-teal-400 transition-colors"
                  size={18}
                />
                <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-1">
                  <span className="text-[10px] text-slate-400 border border-white/10 rounded px-1.5 py-0.5">
                    ⌘ K
                  </span>
                </div>
                <input
                  ...
                  className="w-full pl-10 pr-12 py-2.5 rounded-xl bg-slate-900/50 border border-white/5 text-slate-200 focus:outline-none focus:border-teal-500/50 focus:ring-1 focus:ring-teal-500/50 placeholder:text-slate-600 transition-all text-sm relative z-10"
                />
              </div>
            </div>
            <div className="md:hidden font-bold text-lg capitalize text-white">
              {mobileViewTitle}
            </div>
            ...
            <div className="flex items-center gap-4">
              <button className="p-2.5 text-slate-400 hover:text-white hover:bg-white/10 dark:hover:bg-slate-800 rounded-xl transition-all relative group">
                <Bell size={20} />
                <span className="absolute top-2.5 right-2.5 w-2 h-2 bg-teal-500 rounded-full border-2 border-[#020617] shadow-sm"></span>
              </button>
              ...
              <div className="h-8 w-px bg-white/10 dark:bg-slate-800 mx-1 hidden sm:block"></div>
        ...
        <div
          className={`flex-1 overflow-auto scroll-smooth transition-colors duration-300 ${
            isLaunchpad
              ? "bg-slate-950"
