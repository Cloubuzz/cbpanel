...
import { fetchToppingLinkedItems, fetchToppingList, saveTopping, deleteTopping, fetchMenuItems, type ApiLinkedMenuItem, type ApiToppingListItem } from '../../services/menuItemsApi';
...
const mapToppingToGroup = (item: ApiToppingListItem): ModifierGroup => {
  ...
  const linkedCount = (item as any).LinkedCount ?? (item.ProductName ? 1 : 0);

  return {
    id: String(item.id),
    name: toppingName,
    description: `${linkedCount} Linked Product${linkedCount === 1 ? '' : 's'}`,
...
const mapLinkedItemsToGroup = (group: ModifierGroup, items: ApiLinkedMenuItem[]): ModifierGroup => {
  return {
    ...
    items: items.map((item, index) => {
      ...
      const tId = item.ToppingId ? Number(item.ToppingId) : 0;
...
export const Modifiers: React.FC = () => {
  ...
  useEffect(() => {
    const loadProducts = async () => {
      if (!token) return;
      try {
        const menuItems = await fetchMenuItems(token, { page: 1, pageSize: 1000 });
        const mappedProducts = menuItems.map((item) => {
          const firstSize = Array.isArray(item.sizes) && item.sizes.length > 0 ? item.sizes[0] : null;
          const price = firstSize ? Number(firstSize.price || firstSize.Price || 0) : 0;
          return {
            id: String(item.ID || (item as any).id || ''),
            name: item.Name || (item as any).name || 'Unnamed Product',
            price,
          };
        }).filter((p) => Boolean(p.id && p.name));
        setProducts(mappedProducts);
      } catch (error) {
        console.error('Failed to load menu items for products dropdown:', error);
      }
    };
    loadProducts();
  }, [token]);
  ...
  const handleDelete = async (id: string) => {
    ...
    const isPersisted = groups.some(g => g.id === id);
    if (!isPersisted) {
      setIsEditing(false);
      setCurrentGroup(null);
      return;
    ...
    setIsSaving(true);
    try {
      await deleteTopping(token, Number(id));
      await loadToppings();
      setIsEditing(false);
      setCurrentGroup(null);
    } catch (error) {
      alert(error instanceof Error ? error.message : 'Failed to delete modifier group.');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const groupDesc = currentGroup.description.trim();
      const isRequired = currentGroup.isRequired;
      const isMultiSelect = currentGroup.selectionType === 'multi';
      const isActive = currentGroup.isActive;

      const itemsToSave = currentGroup.items.length > 0
        ? currentGroup.items
        : [{ id: '0', name: groupName, price: 0, productId: '0' }];

      let lastMessage = 'Modifier saved successfully.';

      for (const item of itemsToSave) {
        const mItemId = Number(item.productId || item.id) || 0;
        const itemToppingId = item.toppingId && item.toppingId > 0 ? item.toppingId : 0;
        ...
        const res = await saveTopping(token, payload);
        if (res?.message) {
  ...
  const addProductToGroup = (product: { id: string; name: string; price: number }) => {
    if (!currentGroup) return;
  ...
  const filteredProducts = products.filter(p =>
    ...
    !currentGroup?.items.find(item => item.productId === p.id || item.name.toLowerCase() === p.name.toLowerCase())
  );
