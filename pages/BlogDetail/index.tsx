...
import { addBlog, updateBlog, fetchBlogs } from "../../services/blogsApi";
...
export const BlogDetail: React.FC<BlogDetailProps> = ({ id, onBack }) => {
  ...
  useEffect(() => {
    ...
    fetchBlogs(token)
      .then((blogs) => {
        ...
        if (found) {
          setFormData({
            ...
            excerpt: "",
            category: "Technology",
            status: found.IsActive === 1 ? "Published" : "Draft",
            author: "",
            tags: [],
            featuredImage: found.CoverImage || "",
            seoTitle: "",
            seoDescription: "",
            slug: found.Slug || "",
          });
      ...
      .catch((err) => {
        setFetchError(err instanceof Error ? err.message : "Failed to load blog.");
      })
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setSaveError(err instanceof Error ? err.message : 'Failed to save blog.');
    } finally {
      setSaving(false);
    }
  };
  ...
  if (isFetching) {
    return (
      <div className="p-8 w-full space-y-6 animate-pulse">
        ...
        <div className="h-[500px] bg-slate-100 dark:bg-slate-800 rounded-[40px]" />
      </div>
    );
  }
  ...
  if (fetchError) {
    return (
      <div className="p-8 w-full flex flex-col items-center justify-center py-32 gap-4">
        ...
        <p className="text-slate-700 dark:text-slate-300 font-black">{fetchError}</p>
        <button onClick={onBack} className="text-teal-500 text-sm font-bold uppercase tracking-widest hover:underline">
          Back to Blogs
        </button>
      </div>
    );
    ...
    <div className="p-6 md:p-8 space-y-8 animate-fade-in pb-20 max-w-[1600px] mx-auto min-h-screen">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {["Draft", "Published", "Scheduled"].map((status) => (
                    <button
                      key={status}
                      onClick={() =>
                        setFormData({ ...formData, status: status as string })
                      }
      ...
      {saveError && (
        ...
        >
          ...
          <div className="flex flex-col">
            <span className="font-black text-sm uppercase tracking-widest">Save Failed</span>
