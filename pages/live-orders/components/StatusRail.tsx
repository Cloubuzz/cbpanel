...
export const StatusRail: React.FC<Props> = ({ activeTab, tabData, onTabChange }) => {
  ...
  return (
    <div className="w-16 border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex flex-col items-center py-4 gap-4 z-20">
      ...
      <button 
        onClick={() => navigate('/admin/live-orders/journey')}
        className="w-12 h-12 rounded-xl flex flex-col items-center justify-center gap-1 text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-600 dark:hover:text-slate-200 transition-all"
      >
        <History size={20} />
