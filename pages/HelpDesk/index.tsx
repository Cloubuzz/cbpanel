...
export const HelpDesk: React.FC = () => {
  ...
  return (
    <div className="flex h-full bg-slate-50 dark:bg-slate-950 overflow-hidden font-sans">
          ...
          {filteredTickets.length > 0 ? filteredTickets.map(ticket => (
            ...
            <div className="p-10 text-center">
              ...
              <p className="text-slate-400 text-sm font-medium">
                {tickets.length === 0 ? 'No tickets yet.' : 'No tickets found matching your search.'}
              </p>
            </div>
          )}
        </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden bg-slate-50 dark:bg-slate-950">
        {!selectedTicket ? (
          <div className="flex flex-col items-center justify-center h-full text-center gap-2 text-slate-400 p-10">
            ...
            <p className="text-sm font-medium">{tickets.length === 0 ? 'No tickets yet.' : 'Select a ticket to view details.'}</p>
          </div>
        ) : (
        <>
        {/* Detail Header */}
      ...
      {/* Confirmation Modal */}
      {showConfirmModal.show && selectedTicket && (
