...
const initialNodes: Node[] = [
  { id: '1', type: 'triggerNode', data: { label: 'Cart Abandoned', subLabel: 'User leaves checkout', icon: ShoppingBag }, position: { x: 450, y: 0 } },
  { id: '2', type: 'segmentNode', data: { label: 'High Value Cart', icon: Database, config: { filters: ['Cart > RS 100', 'Currency = PKR'] } }, position: { x: 450, y: 200 } },
  { id: '3', type: 'actionNode', data: { label: 'Send VIP Discount', icon: Mail }, position: { x: 100, y: 450 } },
  { id: '4', type: 'actionNode', data: { label: 'Wait 4 Hours', icon: Clock }, position: { x: 100, y: 620 } },
  { id: '5', type: 'actionNode', data: { label: 'SMS Reminder', icon: MessageSquare }, position: { x: 100, y: 790 } },
  { id: '5b', type: 'actionNode', data: { label: 'Wait 1 Day', icon: Clock }, position: { x: 100, y: 960 } },
  { id: '5c', type: 'actionNode', data: { label: 'Last Chance Email', icon: Mail }, position: { x: 100, y: 1130 } },
  { id: '6', type: 'actionNode', data: { label: 'Send Nudge Email', icon: Mail }, position: { x: 800, y: 450 } },
  { id: '7', type: 'actionNode', data: { label: 'Wait 24 Hours', icon: Clock }, position: { x: 800, y: 620 } },
  { id: '8', type: 'actionNode', data: { label: 'Push Notification', icon: Bell }, position: { x: 800, y: 790 } },
  { id: '9', type: 'segmentNode', data: { label: 'Check: Purchased?', icon: ShoppingBag, config: { filters: ['Order Count > 0'] } }, position: { x: 800, y: 960 } },
  { id: '10', type: 'actionNode', data: { label: 'Send Survey', icon: MessageSquare }, position: { x: 1150, y: 1200 } },
  { id: '11', type: 'actionNode', data: { label: 'Exit Flow', icon: X }, position: { x: 600, y: 1200 } },
];
...
const AutomationDetailContent: React.FC<AutomationDetailProps> = ({ automationId, onBack }) => {
  ...
  const onDrop = useCallback(
    (event: React.DragEvent) => {
      ...
      const meta = event.dataTransfer.getData('application/meta');
  ...
  const handleSave = (isPublish: boolean = false) => {
    alert(`Automation ${isPublish ? 'published' : 'saved as draft'}.`);
  };
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-8 scrollbar-hide">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 gap-2">
              <DraggableBlock label="New Customer" icon={<Users size={16} />} onDragStart={(e) => onDragStart(e, 'triggerNode', 'New Customer', 'trigger')} color="teal" />
              <DraggableBlock label="Cart Abandoned" icon={<ShoppingBag size={16} />} onDragStart={(e) => onDragStart(e, 'triggerNode', 'Cart Abandoned', 'trigger')} color="teal" />
              <DraggableBlock label="Scheduled Time" subLabel="Start at specific date/time" icon={<CalendarClock size={16} />} onDragStart={(e) => onDragStart(e, 'triggerNode', 'Scheduled Start', 'trigger')} color="teal" />
            </div>
          </div>
          ...
          <div className="relative">
            ...
            <div className="grid grid-cols-1 gap-2">
              <DraggableBlock label="Customer Segment" subLabel="Filter by attributes & behavior" icon={<Filter size={16} />} onDragStart={(e) => onDragStart(e, 'segmentNode', 'Customer Segment', 'segment')} color="rose" />
              <DraggableBlock label="Product Segment" subLabel="Filter by purchase history" icon={<ShoppingBag size={16} />} onDragStart={(e) => onDragStart(e, 'segmentNode', 'Product Segment', 'segment')} color="rose" />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 gap-2">
              <DraggableBlock label="Send Email" icon={<Mail size={16} />} onDragStart={(e) => onDragStart(e, 'actionNode', 'Send Email', 'action')} color="blue" />
              <DraggableBlock label="Send SMS" icon={<MessageSquare size={16} />} onDragStart={(e) => onDragStart(e, 'actionNode', 'Send SMS', 'action')} color="blue" />
              <DraggableBlock label="Push Notification" icon={<Bell size={16} />} onDragStart={(e) => onDragStart(e, 'actionNode', 'Push Notification', 'action')} color="blue" />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <DraggableBlock label="Delay" icon={<Clock size={16} />} onDragStart={(e) => onDragStart(e, 'default', 'Wait', 'logic')} color="amber" compact />
              <DraggableBlock label="Split" icon={<GitFork size={16} />} onDragStart={(e) => onDragStart(e, 'default', 'Split', 'logic')} color="amber" compact />
            </div>
          </div>
        ...
        >
          ...
          <Panel position="top-right" className="flex gap-3 mr-4 mt-4">
             <button
                ...
                className="flex items-center gap-2 px-4 py-2 bg-white/80 dark:bg-slate-900/80 backdrop-blur text-slate-700 dark:text-slate-300 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 hover:border-teal-500/50 hover:text-teal-600 dark:hover:text-white transition-colors"
             >
                <Save size={16} />
            ...
            <button
                ...
                className="flex items-center gap-2 px-5 py-2 bg-teal-600 text-white rounded-lg shadow-md hover:bg-teal-500 transition-all font-medium"
            >
                <Play size={16} />
                  ...
                  <div className="bg-rose-50 dark:bg-rose-950/20 border border-rose-100 dark:border-rose-900/50 rounded-xl p-4">
                    ...
                    <div className="space-y-3">
                       <div className="flex items-center gap-2">
                          ...
                          <input type="text" defaultValue="RS 500" className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 text-xs rounded-lg p-2 w-1/3 outline-none focus:border-rose-500" />
                       </div>
                       ...
                       <div className="flex items-center gap-2">
                          ...
                          <input type="text" defaultValue="New York" className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 text-xs rounded-lg p-2 w-1/3 outline-none focus:border-rose-500" />
                       </div>
