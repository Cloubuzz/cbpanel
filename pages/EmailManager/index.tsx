...
export const EmailManager: React.FC = () => {
  ...
  const [history, setHistory] = useState<{blocks: EmailBlock[], settings: GlobalSettings}[]>([{blocks: [], settings: DEFAULT_SETTINGS}]);
    ...
    return (
      <div className="p-8 bg-slate-50 dark:bg-slate-950 min-h-[calc(100vh-80px)] overflow-y-auto">
        <div className="w-full">
          ...
          {templates.length === 0 ? (
            <div className="flex flex-col items-center justify-center text-center gap-2 text-slate-400 p-20">
              <LayoutTemplate size={32} className="opacity-50" />
              <p className="text-sm font-medium">No email templates yet.</p>
            </div>
          ) : (
        ...
        <div className="h-14 bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-4 z-10">
          ...
          <div className="flex items-center gap-2">
             ...
             <button
               ...
               className="flex items-center gap-2 px-3 py-1.5 bg-teal-600 text-white rounded-lg hover:bg-teal-500 transition-all text-xs font-bold shadow-lg shadow-teal-900/20"
             >
               <Save size={14} />
