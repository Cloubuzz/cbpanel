...
export const ReportDetail: React.FC<ReportDetailProps> = ({ onBack }) => {
  ...
  React.useEffect(() => {
    setCurrentPage(1);
  }, [searchQuery, channelFilter, startDate, endDate]);

  const totalPages = Math.max(1, Math.ceil(filteredData.length / PAGE_SIZE));
  const safeCurrentPage = Math.min(currentPage, totalPages);
  const paginatedRows = useMemo(
    () => filteredData.slice((safeCurrentPage - 1) * PAGE_SIZE, safeCurrentPage * PAGE_SIZE),
           ...
           <div className="flex bg-slate-50 dark:bg-slate-950 p-1 rounded-2xl border border-slate-100 dark:border-slate-800">
            ...
            <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
              {filteredData.length > 0
                ? `Showing ${(safeCurrentPage - 1) * PAGE_SIZE + 1}–${Math.min(safeCurrentPage * PAGE_SIZE, filteredData.length)} of ${filteredData.length} records`
                : '0 records'}
            </div>
        </div>
      </div>
        ...
        <AnimatePresence mode="wait">
          {viewType === 'pivot' ? (
            ...
            >
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                    {paginatedRows.map((row, index) => (
                      <tr key={getRowKey(row, (safeCurrentPage - 1) * PAGE_SIZE + index)} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors group">
                        {columnKeys.map((key) => (
              ...
              {totalPages > 1 && (
                <div className="px-6 py-4 border-t border-slate-50 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-4">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                    Showing {(safeCurrentPage - 1) * PAGE_SIZE + 1}–{Math.min(safeCurrentPage * PAGE_SIZE, filteredData.length)} of {filteredData.length} results
                  </span>
                  <div className="flex items-center gap-1">
                    <button
                      onClick={() => setCurrentPage(p => Math.max(1, p - 1))}
                      disabled={safeCurrentPage === 1}
                      className="w-8 h-8 flex items-center justify-center rounded-xl text-slate-400 hover:text-teal-500 hover:bg-teal-500/10 disabled:opacity-30 disabled:cursor-not-allowed transition-all"
                    >
                      <ChevronLeft size={16} />
                    </button>
                    {getPageNumbers().map((p, i) =>
                      p === '...' ? (
                        <span key={`ellipsis-${i}`} className="w-8 h-8 flex items-center justify-center text-slate-400 text-xs select-none">…</span>
                      ) : (
                        <button
                          key={p}
                          onClick={() => setCurrentPage(p as number)}
                          className={`w-8 h-8 flex items-center justify-center rounded-xl text-xs font-bold transition-all ${safeCurrentPage === p ? 'bg-teal-500 text-white shadow-lg shadow-teal-500/20' : 'text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800'}`}
                        >
                          {p}
                        </button>
                      )
